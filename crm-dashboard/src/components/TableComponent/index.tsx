...
import { TableColumn, ExpandableRowConfig } from './types';
...
function DataTable<T>({
	...
	const isServerSidePaginated = totalCount !== undefined && totalCount !== data.length;
	...
	return (
		<div className="w-full">
			<div className="relative border border-gray-200 shadow-sm rounded-lg">
				<div className="overflow-x-auto custom-scrollbar">
					<table className={className.table ?? 'min-w-full whitespace-nowrap'}>
						<thead className={className.tableHeader ?? 'bg-gray-100 sticky top-0 z-10'}>
							<tr>
								{columns?.map((col, idx) => (
									<th
										key={idx}
										className={`${
											...
											`text-left px-4 py-2 text-sm font-medium ${col.align ?? 'left'}`
										} ${col.headerClassName || ''}`}
									>
							...
							{displayData?.length === 0 ? (
								...
								displayData?.map((row, rowIdx) => (
									<React.Fragment key={rowIdx}>
										<tr
											className={`${
												typeof className.tableRow === 'function'
								...
								<td
													key={colIdx}
													className={`${className.tableCell ?? 'px-4 py-2 text-sm'} ${col.className || ''}`}
												>
													{col.render
										...
										{expandableRows && expandableRows.isExpanded(row) && (
											<tr className={className.expendableRow}>
												<td colSpan={columns.length} className={className.expendableRowCell}>
			...
			{pagination && (
				<div
					className={`${className.pagination ?? ''} flex flex-col sm:flex-row justify-between items-center mt-4 gap-4`}
				>
					<PaginationControls
						pageSize={pageSize}
						totalPages={Math.max(
							1,
							Math.ceil((typeof totalCount === 'number' ? totalCount : data.length) / pageSize)
						)}
